...
import { LogOut, Home, Calendar, Users, DollarSign, User } from "lucide-react"
...
export default function ReceptionistLayout({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        ...
        } else if ((session.user as any)?.role !== "RECEPTIONIST") {
    ...
    const menuItems = [
        { name: "Dashboard", icon: Home, href: "/receptionist/dashboard" },
        { name: "My Profile", icon: User, href: "/receptionist/profile" },
        { name: "Appointments", icon: Calendar, href: "/receptionist/appointments" },
        { name: "Patients", icon: Users, href: "/receptionist/patients" },
        { name: "Billing", icon: DollarSign, href: "/receptionist/billing" },
    ]
                ...
                <div className="p-4 border-t border-blue-800 space-y-2">
                    <div className="flex items-center gap-3 p-3 rounded-lg bg-blue-800">
                        <div className="w-8 h-8 bg-blue-400 rounded-full flex items-center justify-center text-sm font-bold">
                            {receptionistName.split(' ').map((n: string) => n[0]).join('').toUpperCase().slice(0, 2)}
                        </div>
                        {sidebarOpen && (
                            <div className="flex-1 min-w-0">
                                <p className="text-sm font-medium truncate">{receptionistName}</p>
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                <div className="bg-white border-b border-gray-200 h-16 flex items-center px-6 shadow-sm">
                    <div className="flex-1">
                        <h2 className="text-xl font-semibold text-gray-900">Reception Portal</h2>
                    </div>
                </div>
